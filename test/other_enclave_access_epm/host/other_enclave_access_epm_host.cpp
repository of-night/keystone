...
unsigned long long start_offset = -1UL;

int main(int argc, char *argv[]) {

    if (argc != 2) {
      printf("Usage: %s <start_offset> \n", argv[0]);
      return 0;
    }

    start_offset = atoi(argv[1]);
    ...
    enclave.init("other_enclave_access_epm_eapp", "eyrie-rt", "loader.bin", params);
    enclave.registerOcallDispatch(incoming_call_dispatch);

    /* We must specifically register functions we want to export to the
       enclave. */
    register_call(OCALL_PRINT_STRING, print_string_wrapper);
    register_call(OCALL_STARTOFFSET_GET, get_dispath_get_wrapper);
...
void get_dispath_get_wrapper(void* buffer) {
  ...
  if (start_offset == -1UL) {
    std::cout << "start_offset error!" << std::endl;
    return;
  }

  if (edge_call_setup_wrapped_ret(edge_call, (void*)&start_offset, sizeof(start_offset))) {
    edge_call->return_data.call_status = CALL_STATUS_BAD_PTR;
  } else {
    edge_call->return_data.call_status = CALL_STATUS_OK;
  }
